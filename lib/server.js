...
module.exports = {
	...
	serve(tree, {port, changePortIfInUse = false, h2 = false, key, cert, acceptRemoteConnections = false}) {
		return Promise.resolve().then(() => {
			const oNeoApp = JSON.parse(fs.readFileSync(tree.proxy.neoApp, 'utf8'));
			const oDestinations = JSON.parse(fs.readFileSync(tree.proxy.neoDest, 'utf8'));
			console.log(oNeoApp)
			...
			if (oNeoApp && oNeoApp.routes) {
				oNeoApp.routes.forEach(function(oRoute) {
					...
					if (oTarget && oTarget.name !== "sapui5") {
					// proxy options
						const oOptions = {};
						...
						if (oDestinations && oTarget.name) {
							...
							if (oDestination) {
								...
								if (oDestination.useProxy) {
									console.log("Using Proxy, setting it to " +tree.proxy.HTTP_PROXY);
									oOptions.agent = new HttpsProxyAgent(tree.proxy.HTTP_PROXY);
								}
						...
						app.use(oRoute.path, proxy({ target: "https://ui5.sap.com", changeOrigin: true, agent : new HttpsProxyAgent(tree.proxy.HTTP_PROXY) }));			
					}
